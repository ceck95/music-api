...
router.get('/', function(req, res) {
  res.render('index', {
    title: 'music.nhutuit.com - NhutUIT'
  });
});
...
router.get('/api/top-music', function(req, res) {
  var options = {
    method: "GET",
    url: "http://m.nhaccuatui.com"
  };

  rp(options).then(function(body) {
      var $ = cheerio.load(body);
      var htmlJson = $('.row').eq(5).text();
      var html = [];
      $('.row').each(function(i, e) {
        var a = $(this).prev();
        if (a.find('.txt-40').html() !== null || a.find('.img-40').html() !== null) {
          html.push('<div class="rawler">' + a.html() + '</div>');
        }
      });
      html.push('<div class="rawler">' + $('.noborder').slice(0).html() + '</div>')
      var content = html.join(' ');
      htmlToJson.parse(content, function() {
        return this.map('.rawler ', {
          'TenSong': function($name) {
            return $name.find('.txt-40 h3 a').text();
          },
          'Image': function($name) {
            return $name.find('.img-40 a img').attr('src')
          },
          'Listener': function($cs) {
            var listener = $cs.find('.txt-40 p img').attr('alt');
            var getlistener = listener.substring(0, listener.length - 10);
            return getlistener;
          },
          'LinkSong': function($cs) {
            var text = $cs.find('.txt-40 h3 a').attr('href');
            // return text;
            // var gettext = text.substring(32, text.length);
            // var bd = gettext.indexOf('.');
            // var link = gettext.substring(bd + 1, gettext.length - 5);
            var url = req.protocol + '://' + req.get('host');
            var textReplace = url + '/playlist/';
            var textFinal = text.replace("http://m.nhaccuatui.com/playlist/", textReplace);
            return textFinal;
          },
        });
      }).done(function(items) {
